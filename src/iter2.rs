...
use crate::{BitBlock, data_block_index, data_block_index2};
...
type LevelIndices<T: SparseHierarchy2> =
    ConstArrayType<
        usize,
...
type LevelIterators<T: SparseHierarchy2> =
    ConstArrayType<
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let level_index = loop {
            ...
            if let Some(index) = last_level_iter.next() {
                ...
                struct V<'b,'a,T: SparseHierarchy2>(&'b mut Iter2<'a, T>); 
                impl<'b,'a,T: SparseHierarchy2> ConstIntVisitor for V<'b,'a,T> {
                    type Out = ();
                    #[inline(always)]
