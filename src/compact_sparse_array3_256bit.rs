//! Experimental 256bit wide nodes. 
//! 
//! On x86 is significantly slower than 64bit(the usual) version.
...
/// Node can be in two modes: "compact" and "full".
/// In "compact" mode - only children stored 
#[repr(C)]
struct NodeHeaderN<const N: usize> {
    mask: Mask,
    mask_block_popcnts: [u8; 4],
...
impl NodeHeader{
    ...
    unsafe fn get_dense_index(&self, index: usize) -> usize {
        ...
        let sub_mask = *self.mask.as_array_ref().get_unchecked(block_index); 
        let block = (sub_mask /*| self.mask_disabler as u64*/) & mask;

        let prev_popcnt = *self.mask_block_popcnts.get_unchecked(block_index) as usize;
        block.count_ones() as usize + prev_popcnt
    }
    
    #[inline]
    pub fn contains(&self, index: usize) -> bool {
        unsafe{ bit_utils::get_array_bit_unchecked(self.mask.as_array_ref(), index) }
    }    
}
...
impl NodePtr {
    ...
    #[inline]
    fn layout<T: NodeChild>(cap: u16) -> Layout {
    ...
    #[inline]
    pub fn new<T: NodeChild>(cap: u16) -> Self {
        unsafe {
            let node = alloc(Self::layout::<T>(cap)) as *mut NodeHeader;
            
            addr_of_mut!((*node).mask).write(Mask::default());
            addr_of_mut!((*node).mask_block_popcnts).write(Default::default());
        ...
        -> (NonNull<T>, /*Option<*/Self/*>*/) 
    {
        ...
        let (header, children_ptr) = self.header_and_children_mut::<T>();
        bit_utils::set_array_bit_unchecked::<true, _>(header.mask.as_array_mut(), index);
        ...
        // update block_popcnts
        for i in ((index/64) + 1)..4{
            header.mask_block_popcnts[i] += 1;
        }
